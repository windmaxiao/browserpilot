...
async def main():
    ...
    try:
        ...
        logger.info("导航结果: success={} title={}", obs.success, obs.title)

        # 2. 获取 Snapshot
        logger.info("=== 步骤 2: 获取页面 Snapshot ===")
        snapshot = await snapshot_gen.generate()
        logger.info("页面: {} | 按钮={} 输入框={} 链接={}",
                     snapshot.title, len(snapshot.buttons),
                     len(snapshot.inputs), len(snapshot.links))

        # 3. 搜索
        logger.info("=== 步骤 3: 输入搜索关键词 ===")
        obs = await tool.input("#kw", "Agentic RPA")
        logger.info("输入结果: success={}", obs.success)

        # 4. 点击搜索按钮
        logger.info("=== 步骤 4: 点击搜索 ===")
        obs = await tool.click("#su")
        logger.info("点击结果: success={}", obs.success)
        ...
        snapshot = await snapshot_gen.generate()
        logger.info("页面标题: {} | URL: {}", snapshot.title, snapshot.url)
