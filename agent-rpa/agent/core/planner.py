...
# 点击/等待 目标在遇到下一个动词、全角句号或半角逗号/分号处截断；
# 动词后要求有空格/冒号，避免误匹配"点击率"等复合词。
# 待解决问题 #49：半角逗号/分号（中英混输常见）也作为边界，避免
# 「点击 登录, 然后搜索」被整体当作目标；但半角 `.` 不作无差别分隔符
# —— 它常见于版本号/日期/URL（v1.2、2024.06.05），仅当其**后跟空白
# 或行尾**（真正的短语分隔：『点击 登录. 然后…』）时才作为边界。
_VERB_BOUNDARY = r"(?=\s*(?:打开|查找|搜索|查询|搜一下|点击|等待|然后|接着|再)|[，。,;]|\.(?=\s|$)|$)"
